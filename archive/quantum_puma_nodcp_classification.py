...
# CLASSIFICATION (AQI) - ResNet18 features only (no DCP)
# ==========================================
...
# ==========================================
# FEATURE EXTRACTION (ResNet18 only)
# ==========================================

def extract_features(dataset_path, csv_path, image_dir, pm25_max=None):
    """Extract ResNet18 features (512-dim per image)"""
    ...
    X = np.array(features_list)   # shape: (N, 512)
...
if __name__ == "__main__":
    ...
    FEATURES_PATH = "./air_pollution_features_resnet.npz"

    # Load or extract features (ResNet18 only, no DCP)
    if os.path.exists(FEATURES_PATH):
        ...
        # Fall back to full cache if available (reuse X, skip DCP)
        FULL_CACHE = "./air_pollution_features.npz"
        if os.path.exists(FULL_CACHE):
            print("Loading X from existing full feature cache (skipping DCP)...")
            data = np.load(FULL_CACHE)
            X = data['X']
            y = data['y']
            PM25_MAX = float(data['PM25_MAX'])
        else:
    ...
    print(f"Input feature dim: {X.shape[1]}")  # should be 512
    ...
    print("=" * 55)
    print("QUANTUM PUMA (ResNet18 only) - CLASSIFICATION RESULTS")
    print("=" * 55)
